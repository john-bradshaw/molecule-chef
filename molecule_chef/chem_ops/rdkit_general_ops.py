...
def form_canonical_smi_frozenmultiset(period_seperated_str):
    return multiset.FrozenMultiset([
        return_canoncailised_smiles_str(get_molecule(mol_str, kekulize=False),
                                                          remove_am=True, kekuleSmiles=False)
        for mol_str in period_seperated_str.split('.')])
